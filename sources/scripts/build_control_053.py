...
"""Build three Local 0.53 consistency-control intensities.
...
MONO_CELL = 616
CURVED_LOWERCASE = "aegnoscbmpqdhur"
CURVED_CAPS = "O0R"

INTENSITIES = {
    "Subtle": {"gravity": 0.026, "drift": 5.0, "terminal": 6.0},
    "Recommended": {"gravity": 0.038, "drift": 9.0, "terminal": 9.0},
    "Strong": {"gravity": 0.052, "drift": 13.0, "terminal": 12.0},
}

# A small shared phase change. Local 0.53 replaces arbitrary per-glyph wobble
# with a repeatable habit: curves arrive slightly late and favor the same
# forward direction. Small differences remain between construction families,
# not between unrelated hashes.
DRIFT_DIRECTIONS = {
    "a": 0.42,
    "e": 0.48,
    "g": 0.38,
...
def add_grotesk_l_foot(glyph, amount: float) -> None:
    ...
    left = round(x_min)
    right = round(x_max)
    reach = round(29 + amount * 1.6)
    height = round(17 + amount * 0.55)
    pen.moveTo((left, 0))
...
def set_metadata(font, family: str, ps_family: str, style: str) -> None:
    font.familyname = f"{family} Control 053"
    font.fontname = f"{ps_family}Control053-{style}"
    font.fullname = f"{family} Control 0.53 {style}"
    font.weight = "Regular"
    font.version = "0.530"
    font.os2_vendor = "SKFT"
    font.comment = (
        "Local 0.53 consistency experiment: shared curve phase, restrained "
        "lower gravity, hyperlegible ambiguity forms, and related terminals."
    )
...
def apply_grotesk_controls(font, settings) -> None:
    ...
    move_nearest_point(
        ...
        settings["terminal"] * 0.44,
    )
    move_nearest_point(
        font["c"], 477, 405,
        -settings["terminal"] * 0.9,
    ...
    move_nearest_point(
        font["t"], 254, 0,
        -settings["terminal"] * 0.78,
        settings["terminal"] * 0.34,
    )
    add_grotesk_l_foot(font["l"], settings["terminal"])
    slant_descender(font["q"], settings["terminal"] * 1.5)
    kick_r_leg(font["R"], settings["terminal"] * 1.2)
...
def apply_mono_controls(font, settings) -> None:
    ...
    move_nearest_point(
        font["e"], 533, 242,
        -settings["terminal"] * 1.05,
        settings["terminal"] * 0.42,
    )
    move_nearest_point(
        font["c"], 516, 392,
        -settings["terminal"] * 0.86,
        settings["terminal"] * 0.38,
    )
    move_nearest_point(
        font["r"], 559, 422,
        -settings["terminal"] * 0.9,
    ...
    slant_descender(font["q"], settings["terminal"] * 2.2)
...
def build_family(
    ...
    for style, settings in INTENSITIES.items():
        ...
        sfd_path = BUILD / f"{ps_family}Control053-{style}.sfd"
        ttf_path = BUILD / f"{ps_family}Control053-{style}.ttf"
